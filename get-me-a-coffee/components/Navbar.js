...
const Navbar = () => {
  ...
  return (
    <nav className='bg-black text-white flex justify-between items-center px-4 md:h-16 flex-col md:flex-row'>
      <Link href={'/'} className="logo font-bold text-xl my-3 md:my-0">GetMeACoffee!</Link> {/* not added img */}
        ...
        {session && <> 
          ...
          <div id="dropdown" className={`z-10 ${showdropdown ? "" : "hidden"} absolute w-full rounded-lg shadow bg-[#452B1F]`}>
              <ul className="py-2 text-sm" aria-labelledby="dropdownDefaultButton">
                <li>
                  <Link href="/dashboard" className="block px-4 py-2 hover:bg-[#786C3B]">Dashboard</Link>
                </li>
                <li>
                  <Link href={`/${session.user.name}`} className="block px-4 py-2 hover:bg-[#786C3B]">Your Page</Link>
                </li>
                <li>
